...
"""
  ...
  node(POD_LABEL) {
    ...
    stage('Build with Kaniko') {
      container('kaniko') {
        sh '/kaniko/executor -f `pwd`/Dockerfile -c `pwd` --skip-tls-verify --destination=harbor.harbor/py-bot:latest --destination=harbor.harbor/py-bot:v$BUILD_NUMBER'
      }
    }
    stage('Deploy and Kustomize') {
      container('kustomize') {
        sh "kubectl -n ${JOB_NAME} get pod"
        sh "kustomize edit set image harbor.harbor/py-bot:v${BUILD_NUMBER}"
